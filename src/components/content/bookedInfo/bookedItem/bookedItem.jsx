...
const BookedItem = ({item, color}) => {
    ...
    return (
        <div className={cx([styles.booked_item_container, itemOlderThenNow(item.dateTime) && styles.expired_item])}
            ...
            <div className={styles.booked_item_text_container}>
                ...
                <div className={styles.booked_item_date}>{styleDateTime(item.dateTime)}</div>
                <div
                    className={styles.booked_item_date}>{item.dateTime.split('T')[1].substring(0, 5)}</div>
            </div>
        </div>
    )
}
